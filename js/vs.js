...
async function initVS() {
  try {
    ...
    // Used for creator-only "Start rematch" behavior.
    creatorPlayerId = await getVSIndexCreatorPlayerId(roomId).catch(() => null);
    if (!creatorPlayerId) {
      creatorPlayerId = await getVSIndexCreatorPlayerId(roomId).catch(() => null);
    }
    ...
    // Rematch button — creator can start rematch; others can request.
