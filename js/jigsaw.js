...
export function generateEdges(cols, rows) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      // Each stored dir means "+1 = tab protrudes OUTWARD from THIS piece".
      // Shared edges must be complementary: the hEdge dir is from the TOP piece's
      // perspective (tab up = +1). The BOTTOM piece sees that same edge as its top,
      // which must be opposite (-t.dir = tab down for bottom piece = slot from above).
      // Similarly for vEdges: l.dir is from the LEFT piece; RIGHT piece flips it.
      edges.push({
        top:        -t.dir,  bottom:     b.dir,   // top piece's bottom = b.dir; bottom piece's top = -t.dir
        left:       -l.dir,  right:      r.dir,   // left piece's right = r.dir; right piece's left = -l.dir
 ...
 *
 * In drawEdge, the normal is (-ey*dir, ex*dir). For a clockwise path this
 * points INWARD, so we negate all edge dirs to make +1 = tab protrudes OUTWARD.
 *
 *   Top    (L→R): ex=1,ey=0 → normal=(0, dir)  → negate: dir=-edges.top    → up   for top=+1 ✓
 *   Right  (T→B): ex=0,ey=1 → normal=(-dir, 0) → negate: dir=-edges.right  → right for right=+1 ✓
 *   Bottom (R→L): ex=-1,ey=0→ normal=(0,-dir)  → negate: dir=-edges.bottom → down  for bottom=+1 ✓
 *   Left   (B→T): ex=0,ey=-1→ normal=(dir, 0)  → negate: dir=-edges.left   → left  for left=+1 ✓
 */
